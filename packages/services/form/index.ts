...
class FormService {
  ...
  public async getFormById(payload: GetFormByIdInputType) {
    ...
    const { id, title, description, views, isPublished, createdAt, updatedAt } = rows[0]!;
  ...
  public async updatePublishStatus(payload: UpdatePublishStatusInputType) {
    const { formId, isPublished } = await updatePublishStatusInput.parseAsync(payload);
    ...
    return {
      success: true,
      message: isPublished ? "Form published successfully" : "Form unpublished successfully",
    };
  }
  ...
  public async getRecentForms(payload: GetRecentFormsInputType) {
    ...
    const forms = await db
      ...
      .where(eq(formsTable.createdBy, userId))
      .groupBy(formsTable.id, formsTable.title, formsTable.isPublished, formsTable.updatedAt)
